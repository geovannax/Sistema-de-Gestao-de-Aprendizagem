...
O app **accounts** estende o `User` padrão do Django com preferências persistidas em JSON, implementa o middleware responsável por gerenciar cookies entre requisições, conecta os eventos de login e logout a comportamentos específicos da plataforma e integra o login social via Google (`django-allauth`).
